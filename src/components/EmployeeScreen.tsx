    ...
    <div className="min-h-screen bg-[#F8FAFC] pt-24 pb-32 px-6">
        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-12 gap-6">
          <div className="flex items-center gap-6 w-full sm:w-auto">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                 ...
                 <p className="text-[10px] font-bold text-slate-400 tracking-[0.3em] uppercase underline underline-offset-4 decoration-indigo-200">System Directory</p>
              </div>
              <h2 className="text-3xl sm:text-4xl font-headline font-bold text-[#0D1C32] tracking-tighter">Organization Members</h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
             <button 
               ...
               className="flex-1 sm:flex-none px-6 py-4 bg-[#0D1C32] text-white font-black uppercase tracking-widest text-xs rounded-2xl shadow-xl shadow-[#0D1C32]/10 hover:opacity-90 active:scale-95 transition-all flex items-center justify-center gap-2"
             >
               <Icons.Plus size={18} />
             ...
             <div className="hidden md:block text-right">
                ...
                <p className="text-2xl font-headline font-bold text-[#0D1C32]">{users.length}</p>
             </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div key={emp.uid} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm relative group hover:shadow-2xl transition-all overflow-hidden hover:-translate-y-1">
               <div className="flex items-center gap-5 mb-8">
                <div className="relative">
                  {emp.photoURL ? (
                    ...
                    <div className="w-20 h-20 rounded-3xl bg-[#0D1C32] flex items-center justify-center text-white text-2xl font-bold shadow-lg relative z-10">
                      {emp.displayName?.[0] || 'U'}
                    </div>
                  )}
                  ...
                  <div className="absolute inset-0 bg-indigo-500/20 blur-2xl rounded-full scale-110 opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
                <div>
                  <h3 className="font-headline font-bold text-xl text-on-surface line-clamp-1">{emp.displayName}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <Icons.Shield size={12} className="text-indigo-500" />
                    <p className="text-[10px] font-black text-[#0D1C32] uppercase tracking-[0.2em]">{emp.role || 'Clerk'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl group/item hover:bg-white hover:shadow-inner transition-all border border-transparent hover:border-slate-100">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-400 group-hover/item:text-indigo-500 transition-colors">
              ...
              <div className="mt-10 flex gap-3">
                <button 
                  ...
                  className="flex-1 py-4 bg-slate-50 text-[#0D1C32] rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-indigo-50 hover:text-indigo-600 transition-all border border-transparent hover:border-indigo-100"
                >
                  Profile
                </button>
                <button 
                  ...
                  className="flex-1 py-4 bg-[#0D1C32] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-all shadow-xl shadow-[#0D1C32]/10"
                >
                   Permissions
                </button>
              </div>
              ...
              <div className="absolute top-0 right-0 p-10 opacity-[0.03] group-hover:opacity-[0.08] transition-all pointer-events-none translate-x-4 -translate-y-4 group-hover:translate-x-0 group-hover:translate-y-0 text-[#0D1C32]">
          ...
          {users.length === 0 && (
            <div className="col-span-full py-32 text-center bg-white rounded-[3rem] border border-slate-100 shadow-sm">
              ...
              <h3 className="text-2xl font-headline font-bold text-[#0D1C32] mb-2">No Staff Added</h3>
              ...
              <button 
                onClick={() => (window as any).setScreen('INVITE')}
                className="px-10 py-5 bg-[#0D1C32] text-white font-black uppercase tracking-[0.2em] text-sm rounded-3xl shadow-2xl hover:scale-105 active:scale-95 transition-all"
              >
                Add Staff
              </button>
            </div>
      ...
      <AnimatePresence>
        {selectedUser && !isManagingRole && (
          <div className="fixed inset-0 bg-[#0D1C32]/80 backdrop-blur-xl z-[200] flex items-center justify-center p-6">
            <motion.div 
            ...
            >
              <button 
                ...
                className="absolute top-8 right-8 p-3 bg-slate-50 text-slate-400 hover:text-[#0D1C32] rounded-2xl transition-all hover:rotate-90"
              >
                <Icons.Close size={24} />
              </button>
              ...
              <div className="flex flex-col items-center text-center">
                <div className="relative mb-6">
                  {selectedUser.photoURL ? (
                    ...
                    <div className="w-32 h-32 rounded-[2.5rem] bg-[#0D1C32] flex items-center justify-center text-white text-5xl font-bold">
                ...
                <h3 className="text-3xl font-headline font-bold text-[#0D1C32] tracking-tighter mb-1">{selectedUser.displayName}</h3>
                <p className="text-indigo-500 font-black uppercase tracking-[0.2em] text-xs mb-8">{selectedUser.role || 'Clerk'}</p>
                ...
                <div className="w-full space-y-4">
                   ...
                   <div className="p-6 bg-slate-50 rounded-3xl text-left">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Icons.Security size={16} className="text-indigo-500" />
                ...
                <button 
                  ...
                  className="w-full mt-10 py-5 bg-[#0D1C32] text-white rounded-3xl font-black uppercase tracking-[0.1em] text-sm shadow-2xl hover:opacity-90 active:scale-95 transition-all"
                >
        ...
        {isManagingRole && selectedUser && (
          <div className="fixed inset-0 bg-[#0D1C32]/80 backdrop-blur-xl z-[210] flex items-center justify-center p-6">
            <motion.div 
            ...
            >
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setIsManagingRole(false)} className="text-slate-400 hover:text-[#0D1C32]">
                  <Icons.Close size={24} />
                </button>
              </div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  { id: 'clerk', label: 'Staff member', desc: 'Basic access for adding items and sales.' }
                ].map(role => (
                  <button 
                    key={role.id}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <p className={`font-black uppercase tracking-widest text-sm ${newRole === role.id ? 'text-indigo-600' : 'text-slate-400'}`}>{role.label}</p>
                      {newRole === role.id && <Icons.CheckCircle className="text-indigo-600" size={20} />}
                    </div>
              ...
              {newRole === 'warehouse_manager' && (
                <div className="mt-8 space-y-2 animate-in fade-in slide-in-from-top-2">
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm focus:ring-2 focus:ring-indigo-500/10 font-bold"
                  >
              ...
              <div className="mt-10 flex gap-3">
                ...
                <button 
                  ...
                  className="flex-[2] py-5 bg-indigo-600 text-white rounded-3xl font-black uppercase text-xs tracking-widest shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 transition-all"
                >
                  Save Access
                </button>
              </div>
